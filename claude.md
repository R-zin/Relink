...
- Phase 3 (cont., 2026-09-05): **Software milestones complete; backup demo video still outstanding.** AES-256-GCM medical-card crypto (`mobile/lib/crypto/demo_key.dart`, `medical_crypto.dart`; key via `--dart-define=MEDICAL_CARD_DEMO_KEY=<base64 32B>`, missing key → SOS falls back to plaintext, never blocked), backend migration `0002_client_msg_id` (nullable `client_msg_id uuid UNIQUE` on `sos_events`+`reports`, idempotent relay flush), `POST /medical/decrypt` (`backend/app/routers/medical.py` + `services/medical_crypto.py`, auth `demo_pass == DECRYPT_DEMO_PASS` env, 200/401/422), two-way community gossip wired (`MeshManager.broadcastMessage` in submit flows; incoming REPORT/MISSING_PERSON/SHELTER → new SQLite `community_items` table v2 → offline map; peer-greeting burst-sync of last 10 on connect), UI trust signals (AppBar mesh radar pill, card origin tags `📡 via Mesh (N hops)` / `🌐 Cloud Verified`, 6s relay snackbar on SOS forward). Verified: `flutter analyze` clean, 34/34 Flutter tests, 37/37 backend tests, deterministic Dart→Python AES-GCM golden vector (`mobile/tool/gen_golden_vector.dart` + `backend/tests/golden_vector.json`). Deviations: (1) `MeshTransportApi` abstract interface extracted in `nearby_transport.dart` (Nearby plugin needs a platform channel absent in unit tests; FakeTransport drives the flooding engine); it now also exposes `eventLog` + `sendTestMessage` so the debug viewer types against the interface. (2) `onPayloadReceived` typedef changed `void Function` → `Future<void> Function` so tests await full DB handling (BLE path ignores the return). (3) sqflite_ffi named test DBs persist on disk across runs — `freshDb`/`freshDao` now `deleteDatabase` first (was silently dedup-dropping the very messages under test). (4) Debug viewer field typed `MeshTransportApi?` (was concrete `NearbyTransport?`). Env: backend `.env` holds `MEDICAL_CARD_DEMO_KEY=AvhVqE/lK/Jv/o5kalpkYjBKHJSxolRrw9j52m1qqCQ=` + `DECRYPT_DEMO_PASS=relink-demo` (gitignored). Alembic head = 0002 on live `relink` DB. **Incomplete:** `demo/phase3_mesh_demo.mp4` backup recording (needs 2 physical phones — user action); AES-GCM encryption path verified in unit tests + golden vector but NOT yet on physical hardware (phone build lacks the `--dart-define`); `flutter_local_notifications` high-priority alert channel is Phase 4 scope (untouched).
- Phase 4: **Done (2026-09-05) — real-time values only, no demo data anywhere in the pipeline.** Backend: telemetry services in `backend/app/services/external_apis/` (GloFAS river discharge 7-day forecast, Open-Meteo Forecast weather, Marine coastal swell, NASA EONET events — all live fetches with per-source stale flags + graceful degradation, never blocking; static seeded dams dataset `app/data/dams_kerala.json`; Copernicus GFM flood-extent as a labeled fixture with observation timestamp, NOT presented as real-time), `stats_service.py` (cached metrics, stale-tracking), `ai_review.py` (rule-based risk synthesis — `LLM_API_KEY` is empty in `.env` so the deterministic rule-based path is the ACTIVE one; LLM branch exists but untested), `alerts_service.py` (NDMA Sachet RSS parse → `alerts_cache` table, migration `0003_alerts_cache`, alembic head = 0003), APScheduler pollers in `app/jobs/` wired into FastAPI lifespan (10-min stats, 15-min alerts). New routers: `GET /stats`, `GET /stats/ai-review`, `GET /alerts?state=` — response shapes settled (`StatsOut.metrics{glofas,weather,dams,gfm,marine,eonet}`, `AiReviewOut{risk_tag,summary_text,source}`, `AlertOut`). Mobile: `flutter_local_notifications` high-priority `emergency_alerts` channel (Red/Orange severity heads-up), `NotificationService` + `AlertPoller` (foreground poll → local OS notification, works without FCM), real Alerts screen (verbatim Sachet text, severity tags) and Stats screen (AI review card, river discharge chart, rainfall, dams with above-danger red flag). Dashboard (`dashboard/`, NEW React 19 + Vite app): monochrome-minimalist incident command center — Header (UTC clock, BACKEND:LIVE/DOWN, pulsing ACTIVE SOS count), left rail (SOS feed with plaintext medical card + **inline 1-click AES-GCM decrypt** via `POST /medical/decrypt`, "Decrypted on view — never stored" indicator; missing persons), center Leaflet map (monochrome OSM tile filter, layer toggles sos/shelters/clusters/missing/gfm, GFM labeled "latest satellite pass, not live"), right rail (AI review, river/rainfall/dams telemetry, NDMA alerts verbatim), status bar. Polls all endpoints every 12s with per-endpoint error isolation. Verified: 43/43 backend tests, 37/37 Flutter tests, `flutter analyze` clean, `npm run build` clean (chunk-size advisory only), E2E at three levels — curl (all 8 dashboard-consumed endpoints live), headless-Chrome DOM dump (data-bearing markup), 1600×1000 screenshot (full console renders against live DB: 2 active SOS with real medical cards, Mullaperiyar 92.7% red-flagged, verbatim Red alert). Deviations: (1) **Tailwind CSS v4 added to dashboard deps** (`tailwindcss` + `@tailwindcss/vite`) — components were authored with Tailwind utilities per the Superdesign spec but the plugin/import was initially missing, silently emitting no styles; fixed, rebuilt, re-screenshotted. (2) `.gitignore` gained `.superdesign` (design-tool state, not repo content). (3) AI review is rule-based in production until `LLM_API_KEY` is set — surfaced in UI as `source: rule-based`. (4) Dashboard `.env`-free by default: `VITE_API_BASE_URL ?? http://localhost:8000`, `VITE_REGION ?? 'Kochi, Kerala'`. Env: no new secrets beyond Phase 3's gitignored `backend/.env`; `backend/.env.example` documents the new optional keys (`SACHET_RSS_URL`, `OPEN_METEO_BASE_URL`, `NASA_EONET_URL`). **Incomplete for demo day:** FCM background push is NOT wired (foreground poll + local notification only — background/killed-app push via `FCM_SERVER_KEY` is deferred; the plan's Demo-Day warning about testing backgrounded push applies when FCM is added); Phase 3 backup mesh video still outstanding (user action); offline map tile pre-caching for venue not done. **Post-commit build fix (separate commit):** `mobile/android/app/build.gradle.kts` gained `isCoreLibraryDesugaringEnabled = true` + `coreLibraryDesugaring("com.android.tools:desugar_jdk_libs:2.1.5")` — required for `flutter_local_notifications` (uses `java.time` on all API levels); the AAR-metadata check fails any physical-device build without it. Found only when building a device APK (unit/widget tests never exercise it); it rode in the Phase-4 device build but was committed after `4c59b34`.
