...
2. **Live Map & Crowdsourced Layers** *(replaces the old separate "clustering-based reporting" and "shelter mapping" features — merged into one map feature with per-type trust signals)*:
   ...
   - One map view (`flutter_map` on mobile, Leaflet on dashboard) with toggleable layers: relief camps/shelters, flooded/blocked roads & other hazards, missing persons (last-seen).
...
**Scope:** External hazard API integration (Section 7 table) + AI review endpoint, Sachet RSS polling + FCM push as true system-tray notifications (Section 3.3) with high-priority channel, React dashboard (map + charts + confirm counts + decrypt view), seed mock historical data so the dashboard doesn't look empty, rehearse triggering one live report during the demo. Social signal monitoring (Section 3, optional) only if time remains after everything else here is solid.
